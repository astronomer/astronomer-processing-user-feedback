...
- **Anyscale API Token**: Obtain your API token either by using the [Anyscale CLI](https://docs.anyscale.com/1.0.0/reference/cli/quickstart/) or through the [Anyscale console](https://console.anyscale.com/v2/api-keys?api-keys-tab=platform).
...
3. Access the Airflow UI at `localhost:8080` (log in with `admin` as the username and `admin` as the password) and follow the DAG running instructions in the [Running the DAGs](#running-the-dags) section of this README. You can run and develop DAGs in this local Airflow environment.
